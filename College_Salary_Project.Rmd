...
On average, each additional GPA point is associated with about **$`r format(round(gpa_coef, 0), big.mark=",")`** more in annual salary. That means the difference between a 3.0 and 4.0 GPA translates to roughly **$`r format(round(gpa_coef, 0), big.mark=",")`** per year. Over a career, that certainly adds up. But here's the catch: GPA alone only explains about **`r round(gpa_r_squared * 100, 1)`%** of the variation in salaries. That means `r round((1-gpa_r_squared) * 100, 1)`% of what determines your salary comes from other factors.

```{r gpa_by_major_plot, fig.cap="Relationship between GPA and salary varies significantly across majors", fig.height=6}
...
So should you be hyper-anxious about every assignment? Our data suggests a more balanced approach. Yes, aim for good grades—they do correlate with higher earnings. But obsessing over the difference between an A- and an A probably isn't worth the mental health cost. Your energy might be better spent on internships, networking, and developing practical skills.
...
```{r major_salary_plot, fig.cap="Average salaries vary dramatically by major category", fig.height=7}
...
Does this mean everyone should become an engineer? Not necessarily. Job satisfaction, work-life balance, and personal fulfillment matter too. But if salary is a priority for you, the data is clear: some majors offer significantly better financial outcomes than others. The key is making an informed choice—knowing the trade-offs you're making.

## The Uncomfortable Truth: The Gender Pay Gap Persists

Perhaps the most troubling finding in our analysis is the persistent gender pay gap. Even when controlling for major and GPA, men earn significantly more than women.
...
Overall, men in our dataset earn an average of **$`r format(round(male_salary, 0), big.mark=",")`** compared to **$`r format(round(female_salary, 0), big.mark=",")`** for women—a gap of **$`r format(round(gender_gap, 0), big.mark=",")`**, or **`r round(gender_gap_pct, 1)`%**. That's over $6,000 per year that compounds throughout a career, translating to hundreds of thousands of dollars over a lifetime.

```{r gender_by_major_plot, fig.cap="Gender pay gap exists across nearly all major categories", fig.height=6}
...
The gap exists across nearly every major. In Engineering, men earn $120,000 on average while women earn $111,363—an $8,638 difference. In Business, men make $97,414 compared to women's $89,420—a $7,994 gap. Even in fields like Education, traditionally dominated by women, men still earn about $2,784 more on average.

As a Statistics major myself, I looked specifically at Computers & Mathematics graduates. Women in this field earn an average of $96,514, while men earn $100,450—a 4.1% gap. While this is smaller than in some other fields, it's still significant and concerning.

## What We Can (and Can't) Predict About Your Salary

So how well can GPA, major, and gender predict what you'll actually earn? The honest answer: not as well as you might hope.
...
example_grads <- data.frame(
  ...
  MajorCategory = c("Engineering", "Education", "Business", "Computers & Mathematics"),
  Sex = c("M", "F", "M", "F")
)
...
```{r prediction_plot, fig.cap="Prediction intervals are wide, showing substantial unexplained variation"}
...
ggplot(salary_data, aes(x = predicted, y = Salary)) +
  ...
  labs(title = "Predicted vs. Actual Salary",
       subtitle = "Red line shows perfect prediction; substantial scatter indicates unexplained variation",
       x = "Predicted Salary", 
       y = "Actual Salary") +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold", size = 14))
...
What's missing from our model? Plenty:
...
- **Graduate education:** An MBA or master's degree significantly impacts earning potential
- **Negotiation skills:** The ability to effectively negotiate salary offers matters immensely
- **Company size and type:** Large corporations typically pay more than startups
...
**Third,** acknowledge and address the gender pay gap. If you're a woman, know that you're likely to face salary disparities—but knowledge is power. Learn to negotiate effectively, research industry salary standards, and don't be afraid to advocate for fair compensation. If you're a man, be aware of these disparities and support equity in your workplace.

**Finally,** remember that your undergraduate credentials are just the beginning. Where you work, how you network, what additional skills you develop, and how you negotiate will all profoundly shape your career trajectory. Your GPA and major open certain doors, but you determine which ones to walk through—and how far you go once you're inside.
